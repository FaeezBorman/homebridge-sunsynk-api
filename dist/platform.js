...
async function fetchRSData() {
    let res = {};
    const inverter = new modbus_serial_1.default();
...
async function fetchFlowData(userData, plantId) {
    ...
    const plants = await (0, node_fetch_1.default)('https://pv.inteless.com/api/v1/plants?page=1&limit=10&name=&status=&type=-1&sortCol=createAt&order=2', requestOptions);
...
async function writePowerFlowSvg(data) {
    ...
    let status = await eskom_loadshedding_api_1.Status.getStatus();
    if (status > 0) {
        const sub = await eskom_loadshedding_api_1.Search.searchSuburbs('Constantia Kloof', 1);
        const { schedule } = await eskom_loadshedding_api_1.Schedule.getSchedule(sub[0].id, status);
  ...
  <g transform="matrix(1.48 0 0 1.48 561.41 257.94)" style=""  >
      <text xml:space="preserve" font-family="'Open Sans', sans-serif" font-size="47" font-style="normal" font-weight="normal" style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-dashoffset: 0; stroke-linejoin: miter; stroke-miterlimit: 4; fill: rgb(225,22,22); fill-rule: nonzero; opacity: 1; white-space: pre;" ><tspan x="-222.08" y="14.76" >${status == eskom_loadshedding_api_1.LoadsheddingStage.UNKNOWN ? '' : status}</tspan></text>
  </g>
  ...
  `;
    (0, fs_1.writeFile)('sunsynk-info.svg', svg, () => {
        //console.log('wrote animation file ',process.cwd()+'/sunsynk-info.svg')
        (0, svg2img_1.default)(svg, (err, buffer) => {
            console.log('error converting to png', err);
            (0, fs_1.writeFile)('sunsynk-info.png', buffer, () => { });
        });
    });
}
